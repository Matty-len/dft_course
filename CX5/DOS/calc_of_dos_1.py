...
# Density of States
plt.subplot(211)
slab, calc = restart('top.gpw') # WE mad platinum slab 
...
plt.subplot(212)
c_mol = GPAW('CO.gpw')  # so we have this molecule
tot = 0
for n in range(2, 9):
    print('Band', n)
    # PDOS on the band n
    wf_k = [kpt.psit_nG[n] for kpt in c_mol.wfs.kpt_u] #WTF
    P_aui = [[kpt.P_ani[a][n] for kpt in c_mol.wfs.kpt_u] for a in range(len(molecule))] #WTF
